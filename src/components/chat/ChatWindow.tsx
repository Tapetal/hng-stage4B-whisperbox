...
function isImageFile(file: File) {
  ...
  return /\.(gif|jpe?g|png|webp|svg)$/i.test(file.name);
}
...
export default function ChatWindow({ contact, me, incomingMessage, isContactOnline = false, onBack }: Props) {
  ...
  return (
    <div className="flex flex-col h-full min-h-0 bg-zinc-50 dark:bg-[#0f0f10] transition-colors">
      <header className="flex items-center gap-3 px-4 md:px-6 py-3 md:py-4 border-b border-zinc-200 dark:border-white/5 bg-white/80 dark:bg-[#1a1a1c]/80 backdrop-blur-md sticky top-0 z-20 flex-shrink-0">
        <button
          onClick={onBack}
        ...
        <div className="relative">
          <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${avatarGradient(contact.id)} flex items-center justify-center text-white font-bold shadow-lg`}>
            {initials}
          </div>
          <div
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-widest text-zinc-400 dark:text-zinc-500">
            <span className="opacity-80">🔒 End to End Encryption</span>
          </div>
        </div>
      ...
      <div className="flex-1 min-h-0 overflow-y-auto px-3 sm:px-4 md:px-8 py-4 md:py-6 space-y-6 scrollbar-hide">
      ...
      <footer className="p-3 sm:p-4 md:p-6 bg-gradient-to-t from-zinc-50 dark:from-[#0f0f10] via-zinc-50 dark:via-[#0f0f10] to-transparent flex-shrink-0">
        <form onSubmit={handleSend} className="max-w-4xl mx-auto space-y-3">
          ...
          {pendingFile && (
            <div className="flex items-center gap-3 rounded-2xl border border-zinc-200 dark:border-white/10 bg-white dark:bg-[#1a1a1c] p-3 shadow-sm dark:shadow-lg">
              <div className="w-16 h-16 rounded-xl border border-zinc-200 dark:border-white/10 bg-zinc-100 dark:bg-black/20 flex items-center justify-center overflow-hidden flex-shrink-0">
                {pendingFileUrl && isImageFile(pendingFile) ? (
                  <img
                    src={pendingFileUrl}
              ...
              <div className="min-w-0 flex-1">
                <p className="text-sm font-semibold text-zinc-900 dark:text-zinc-100 truncate">{pendingFile.name}</p>
                <p className="text-xs text-zinc-500">{Math.max(1, Math.round(pendingFile.size / 1024))} KB</p>
              </div>
              <button
                type="button"
                onClick={clearPendingFile}
                className="w-8 h-8 rounded-lg text-zinc-500 hover:text-zinc-900 dark:hover:text-white hover:bg-zinc-100 dark:hover:bg-white/5 transition-colors flex items-center justify-center"
          ...
          <div className="flex-1 relative">
            <textarea
              ...
              rows={1}
              disabled={sending}
              className="scrollbar-hide block w-full h-12 min-h-12 max-h-32 bg-white dark:bg-[#1a1a1c] border border-zinc-200 dark:border-white/5 rounded-2xl px-4 sm:px-5 py-3 text-sm text-zinc-900 dark:text-white placeholder-zinc-400 dark:placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-brand-500/50 transition-all resize-none overflow-y-auto leading-6 disabled:opacity-60 shadow-sm dark:shadow-2xl"
      ...
      <div className={`max-w-[85%] md:max-w-[70%] ${isMine ? 'items-end' : 'items-start'} flex flex-col gap-1`}>
        ...
        >
          {msg.decryptionFailed ? (
            ...
            <span className="whitespace-pre-wrap">{msg.plaintext}</span>
          )}
        </div>
...
function isImageMessage(msg: DecryptedMessage) {
  return (
    msg.mimeType?.startsWith('image/') ||
    /\.(gif|jpe?g|png|webp|svg)$/i.test(msg.fileName || '')
  );
}
    ...
    return (
      <a href={msg.fileUrl} download={msg.fileName} className="block">
        <img
          ...
          className="max-w-[240px] md:max-w-xs rounded-xl border border-zinc-200 dark:border-white/10 object-cover"
        />
        <p className="mt-2 text-xs opacity-80 truncate">
          {msg.fileName}
        </p>
      </a>
    );
